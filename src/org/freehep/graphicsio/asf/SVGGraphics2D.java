...
public class SVGGraphics2D extends AbstractGraphics2D
    ...
    public void stream(Writer writer, boolean useCss)
        ...
        Element svgRoot = getRoot();        
  /*FIXME ASF      int i = 0;
        while (svgRoot.getAttributes().getLength() != 0)
        {
        	String n = svgRoot.getAttributes().item(i).getNodeName();
        	svgRoot.removeAttribute(n);        	
        }
      */
        svgRoot.removeAttribute("stroke");        
        /*svgRoot.setAttribute("PBA_X", String.valueOf(this.getTransform().getTranslateX()));
        svgRoot.setAttribute("PBA_Y", String.valueOf(this.getTransform().getTranslateY()));*/
            
        //FIXME ASF stream(svgRoot, writer, useCss, false);
        stream(svgRoot, writer, useCss, true);
    }
