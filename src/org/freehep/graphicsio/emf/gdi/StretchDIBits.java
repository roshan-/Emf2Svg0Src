 ...
 *          gdi/StretchDIBits.java 9c0688d78e6b 2007/01/30 23:58:16 duns $
 */
public class StretchDIBits extends EMFTag implements EMFConstants {
	...
	public EMFTag read(int tagID, EMFInputStream emf, int len)
		...
		if (tag.width <= 0 || tag.height <= 0) {
			throw new IllegalStateException("Bad dimensions:" + tag.toString());
		}
		...
		BlendFunction blend = new BlendFunction(AC_SRC_OVER, 0, 0xFF, AC_SRC_ALPHA);
		...
		tag.image = EMFImageLoader.readImage(tag.bmi.getHeader(), tag.width,
				tag.height, emf, len - 72 - BitmapInfoHeader.size, blend);
	...
	public void write(int tagID, EMFOutputStream emf) throws IOException {
		...
		ImageGraphics2D.writeImage((RenderedImage) image,
	...
	public void render(EMFRenderer renderer) {
		...
		// pixels in a
		// DIB to the specified destination rectangle. If the destination
		// rectangle is larger
		// than the source rectangle, this function stretches the rows and
		// columns of color
		// data to fit the destination rectangle. If the destination rectangle
		// is smaller
		// than the source rectangle, this function compresses the rows and
		// columns by using
